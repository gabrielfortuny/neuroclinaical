...
def fetch_graph_data(patient_id):
    ...
    for seizure in patient.seizures:
        seizure_data = {
            ...
            "electrodes": [
                electrode.name for electrode in seizure.electrodes
            ],
        }
...
def make_plot2(patient_id: int, screen: int, view_seizure_length: int, view_soz_heatmap: int, view_drug_admin: int) -> Image.Image:
    """
        ...
        if view_seizure_length == 1:
            for i, day in enumerate(all_days):
                ...
                for j, length in enumerate(lengths):
                    plt.bar(day, length, bottom=bottoms[i], label=f'Seizure {j+1}' if i == 0 else "",
    ...
    if screen == 2:
        ...
        for day in all_days:
            ...
            for i, seizure in enumerate(day_seizures):
                seizure_time = datetime.strptime(seizure['seizure_time'], "%H:%M:%S").time()
                x_value = (day - 1) * 24 + seizure_time.hour + seizure_time.minute / 60 + seizure_time.second / 3600
                ...
                xtick_labels.append(seizure_time.strftime('%H:%M'))  # Only show time, not day
        ...
        for i, (tick, label) in enumerate(zip(xticks, xtick_labels)):
            if abs(xticks[i] - xticks[i - 1]) < max(xticks)/10:  # Check if labels are too close (conflict)
                xticks[i] += max(xticks)/30  # Move the current label to the right by a fixed offset (e.g., 1.0 units)
        ...
        plt.xticks(adjusted_xticks, adjusted_xtick_labels, rotation=45, ha='right', fontsize=8)
        ...
        for day in all_days:
            ...
            plt.text(day_start, plt.ylim()[1] * 0.95, f'Day {day}', color='red', ha='right', va='top', rotation=90, fontsize=8)
        ...
        plt.text(last_day_end, plt.ylim()[1] * 0.95, 'End', color='green', ha='left', va='top', rotation=90, fontsize=12)

        if view_drug_admin == 1:
            # Plot drug administration on a secondary y-axis
            ax2 = plt.twinx()  # Create a secondary y-axis
            unique_drugs = list(set(drug['name'] for drug in drugs))  # Get unique drug names
            color_map = plt.colormaps.get_cmap('tab10')  # Use the recommended method to get a colormap
            ...
            for drug in drugs:
                ...
                x_value = (drug['day'] - 1) * 24 + drug_time.hour + drug_time.minute / 60 + drug_time.second / 3600
            ...
            prevXval = -float('inf')  # Initialize to negative infinity to ensure the first x_value is always larger
            prevYval = -float('inf')  # Initialize to negative infinity to ensure the first y_value is always larger
            ...
            for drugs in sorted_time_to_drugs:
                ...
                drug_index = unique_drugs.index(drug['name'])  # Get index for color mapping
                ...
                if (abs(int(drug['mg_administered']) - prevYval) < (max_mg / 10)) and (abs(x_value - prevXval) < max(xticks) / 4):
                    pos = (max_mg / 15)  + prevPos  # Add vertical offset if labels are too close
                    prevPos = pos

                    x_value = prevXpos + max(xticks) / 100
                    prevXpos = x_value
                else:
                    pos = int(drug['mg_administered']) + max_mg / 20  # Default vertical offset
                    prevPos = pos
                    prevXpos = x_value

                prevYval = int(drug['mg_administered'])  # Update previous y_value
                prevXval =  (drug['day'] - 1) * 24 + drug_time.hour + drug_time.minute / 60 + drug_time.second / 3600 # Update previous x_value
                ...
                ax2.scatter(
                    ...
                    color=color_map(drug_index / len(unique_drugs)),  # Assign unique color
                )
                """
                ax2.text(
            ...
            plt.title('Seizure Durations and Drug Administration by Time and Day', fontsize=9)
            for drug in unique_drugs:

                drug_index = unique_drugs.index(drug)

                legend_patches.append(mpatches.Patch(color=color_map(drug_index / len(unique_drugs)), label=drug) ) # Add to legend
