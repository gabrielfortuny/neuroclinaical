...
def upload_report():
    ...
    try:
        ...
        reports_dir = os.path.join(current_app.config['UPLOAD_FOLDER'], 'reports', str(patient_id))
...
def get_report_metadata(report_id):
    ...
    try:
        ...
        result = db.session.execute(
            text("SELECT id, patient_id, summary, created_at, modified_at, filepath, filetype FROM reports WHERE id = :report_id"),
            {"report_id": report_id}
        )
    ...
    try:
        ...
        try:
            if filepath and os.path.exists(filepath):
                ...
                corrected_path = os.path.normpath(os.path.join('/workspaces/project/backend/NeuroClin', filepath))
